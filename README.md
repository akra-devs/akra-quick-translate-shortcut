...
Chrome 내장 `Translator API`를 사용해 현재 페이지의 텍스트를 빠르게 한국어로 번역하고, 다시 원문으로 복구하는 Manifest V3 확장 프로그램입니다.

## 주요 기능

- `Alt+T` 단축키로 페이지 번역과 원문 복구를 토글합니다.
- macOS에서는 `Option+T`로 동일하게 동작합니다.
- 확장 아이콘 클릭 또는 페이지 우클릭 메뉴에서도 토글할 수 있습니다.
- 기본 번역 대상 언어는 한국어(`ko`)입니다.
- 옵션 페이지에서 번역 대상 언어와 상태 오버레이 표시 여부를 설정할 수 있습니다.
...
확장 상세 페이지에서 **Extension options**를 열어 설정할 수 있습니다.
